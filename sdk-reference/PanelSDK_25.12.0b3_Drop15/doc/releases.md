...
## _Panel SDK Drop 15 - 2025.12 BETA 3_
This **pre-release** Panel SDK Drop 15 is aligned with Media Composer 25.12b3.

This release contains additional transport and command APIs and bug fixes.

Previously, the ```DoCommand``` API was limited because the current window context could not be set first to enable the command. This has be fixed by the addition of the ```MakeActiveWindow``` API which can be called before making one or more ```DoCommand``` calls.

Fortuantely, if the current state of the Media Composer windows is correct for the next command, then it will automate the workflow just as if an editor would by typing consecutive shortcuts or hitting Command Palette buttons or selecting an enabled menu item.


**New APIs since 25.6:**
- Same as Panel SDK Drop 14 - 2024.12.3
- ```ShowWindow```
- ```MakeActiveWindow```
...
**Known issues:** 
- ```DoCommand``` is essentially a wrapper that calls the equivalent menu command in Media Composer. Those commands behave _exactly_ like an editor would after selecting the corresponding menu item, Command Palette button or keyboard shortcut. Consequently, many of them will display a modal dialog. The obvious ones have names that end with an elipsis. Each of these commands will need a new API added and implemented for the API to provide the input that would normally be entered by an editor.
- There are many other commands that may not have the elipsis but still display an error dialog. Please report any such issues to MC-PanelSDK@avid.com. There are currently greater than 700 commands returned from GetListOfCommands. Please be patient as we work through these issues. 
- Only one command can be run at time. In the future there will be an API to run a list of commands.
