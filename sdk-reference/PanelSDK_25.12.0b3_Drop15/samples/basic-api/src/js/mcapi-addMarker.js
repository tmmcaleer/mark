...
const page = '<div id="addMarker-sample"> \
		...
		<div>\
			<table>\
				...
				<tr>\
					<td><label for="file-input"><strong>svg annotation</strong><br>Select a file</label></td>\
					<td>\
						<button id="file-upload-btn" style="padding: 6px 12px; background-color: #007BFF; color: white; border: none; border-radius: 4px; cursor: pointer;">Select SVG</button>\
...
fileInput.addEventListener("change", () => {
    ...
    if (file) {
        ...
        reader.onload = function (event) {
            svg_annotation = event.target.result;
            has_svg_annotation = true; // Set the flag to true when a file is loaded
            console.log("SVG annotation loaded successfully.");
            submitButton.disabled = false; // Re-enable submit button after successful reading
        };

        reader.onerror = function (event) {
            console.error("Error reading svg file:", event.target.error);
            svg_annotation = '';
            has_svg_annotation = false; // Reset the flag if there's an error
            submitButton.disabled = false; // Re-enable submit button after error reading
            const errorMessage = document.createTextNode("Error reading SVG file. Please try again.");
            response_container.appendChild(errorMessage);
        };

        submitButton.disabled = true; // Disable submit button while reading the file
        console.log("Reading file:", file.name);
        reader.readAsText(file); // Start reading the file (async)
    } else {
        console.log("No file selected. Resetting svg_annotation.");
        fileNameSpan.textContent = '';
        svg_annotation = '';
        has_svg_annotation = true; // we want to allow adding markers without svg annotation.
    }
});
