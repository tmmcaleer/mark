...
import { GetHostInfoRequest, GetHostInfoRequestBody,  } from '../grpc-web/MCAPI_Types_pb.js';
...
export var load_getHostInfo = function () {
    parameters.innerHTML = ""; //clear old content
    const element = createElementFromHTML(page);
    parameters.appendChild(element);
    submitButton.onclick = null;
    submitButton.onclick = getHostInfo_submit;
}

var getHostInfo_submit = function () {
 
    var request = new GetHostInfoRequest();

    var body = new GetHostInfoRequestBody;
    request.setBody(body);

    mcapiclient().getHostInfo(request, getMetadata(), (err, response) => {
        if (err) {
