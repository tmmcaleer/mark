...
import { SetPositionRequest, SetPositionRequestBody } from '../grpc-web/MCAPI_Types_pb.js';
    ...
    <table>
      ...
      <tr>
        <td colspan="3" style="text-align: left; padding-right: 20px; background-color: #e4f0f5;">
          ...
          &nbsp;&nbsp;
          <label>
            <input type="radio" name="playback" id="setPlayHead" value="set"> Set Playhead
          </label>
        </td>
      </tr>
      <tr><td></td></tr>
...
export function load_mcapi_transport_controls() {
    ...
    submitButton.onclick = () => {
        ...
        else if (document.getElementById('stopRadio').checked) {
            stopPlay_submit();
        }
        else {
            setPlayhead_submit();
...
function setPlayhead_submit() {
  ...
  const body = new SetPositionRequestBody();
  ...
  mcapiclient().setPosition(request, getMetadata(), (err, response) => {
      if (err) {
          showStatus(`Request Failed: ${err.message}`, true);
      } else {
          showStatus('Playhead set!');
      }
  });
}
