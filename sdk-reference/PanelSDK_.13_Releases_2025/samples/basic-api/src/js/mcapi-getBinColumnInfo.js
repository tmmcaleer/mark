...
import { GetBinColumnInfoRequest, GetBinColumnInfoRequestBody } from '../grpc-web/MCAPI_Types_pb.js';
...
<input type="text" id="getBinColumnInfo-input" placeholder="Bin path"></input> \
<br /> \
<br /> \
</div>';


export var load_getBinColumnInfo = function () {
    parameters.innerHTML = ""; //clear old content
    const element = createElementFromHTML(page);
    parameters.appendChild(element);
    submitButton.onclick = null;
    submitButton.onclick = getBinColumnInfo_submit;
}

var getBinColumnInfo_submit = function () {
    const inputText = document.querySelector("#getBinColumnInfo-input").value;

    var request = new GetBinColumnInfoRequest();

    var requestBody = new GetBinColumnInfoRequestBody;
    requestBody.setBinPath(inputText);
    request.setBody(requestBody);

    mcapiclient().getBinColumnInfo(request, getMetadata(), (err, response) => {
        if (err) {
